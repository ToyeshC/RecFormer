...
def evaluate_diversity(category_dict, model, test_data):
    ...
    # Get the correct device for DataParallel models
    ...
    with torch.no_grad():
        for batch_idx, interaction_batch in enumerate(test_data):
            ...
            if isinstance(interaction, dict):
                # Standard format: dictionary of tensors
                for k_name, v in interaction.items():
                    interaction[k_name] = v.to(device)
            elif isinstance(interaction, list):
                # Check if it's a list of tensors (multi-GPU format) or list of dicts
                if len(interaction) > 0 and isinstance(interaction[0], dict):
                    # List of dictionaries format
                    print(f"Debug: Processing as list of {len(interaction)} dictionaries")
                    # Take the first dictionary and move tensors to device
                    interaction = interaction[0]
                    for k_name, v in interaction.items():
                        interaction[k_name] = v.to(device)
                else:
                    # Multi-GPU format: list of tensors
                    print(f"Debug: Processing as list of tensors with {len(interaction)} elements")
                    # Convert list to dict format expected by the model
                    interaction = {
                        'input_ids': interaction[0].to(device),
                        'item_position_ids': interaction[1].to(device),
                        'token_type_ids': interaction[2].to(device),
                        'attention_mask': interaction[3].to(device),
                        'global_attention_mask': interaction[4].to(device)
                    }
            else:
                print(f"Error: Unknown interaction format {type(interaction)}")
                print(f"Interaction content: {interaction}")
                print(f"Interaction keys (if dict): {interaction.keys() if hasattr(interaction, 'keys') else 'No keys'}")
                all_topk_item_indices_list.append(torch.empty(0, k, dtype=torch.long).cpu())
                continue
            
            # Debug: Check interaction format
            if batch_idx == 0:
                print(f"Interaction type: {type(interaction)}")
                print(f"Interaction keys: {interaction.keys() if isinstance(interaction, dict) else 'Not a dict'}")
            ...
            # Debug: Check scores before unsqueeze
